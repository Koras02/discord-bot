...
const quickSort = (arr, l, r) => {
    ...
    l < r && ((i = partition(arr, l, r)), quickSort(arr, l, i - 1), quickSort(arr, i + 1, r));
...
client.on('message', async msg => {
    ...
    if (content.startsWith('지은아') || content.startsWith('지금아')) {
        ...
        if (blacklist.includes(authorid)) return console.log(`blocked user : ${authorid} - ${content}`);
        ...
        } else if (content === '잘 가' || content === '잘가' || content == 'ㅂㅂ' || content == 'ㅂㅇ') {
            msg.react('💜').then(() => {
                ...
                if (action === '추가') {
                    ...
                    try {
                        ...
                        if (result.deletedCount) {
                            client.channels.cache
                                .get(channel)
                                .send(`성공적으로 알림 채널을 삭제했어요.\n채널 ID : ${channel}`)
                                .then(() => {
                                    msg.reply('완료!');
                                });
                        } else {
                            msg.reply('해당 채널이 존재하지 않거나 삭제에 실패했어요.');
                        }
                    } catch (err) {
        ...
        // Info
            ...
            target && (target = target.replace('번째', '').replace('번쨰', '')), +target ? (target = --target) : (target = 0);
        ...
        // Extra Functions
            ...
            if (content.split(' ').length >= 2) {
                if (content.slice(-3) === '-지워') {
                    msg.channel.send(content.slice(0, -3).replace('말해 ', '')).then(() => {
                        try {
                            msg.delete();
                        } catch (err) {
                            msg.channel.send(
        ...
        } else if (content === '힘들다' || content === '힘들어' || content === '나 힘들다' || content === '나 힘들어') {
            const songs = [
                'uZf9Q_SOzvY',
                'Tqudgg0aBAI',
                'SfeaTW4bcAw',
        ...
        // mini games
            ...
            Promise.all([msg.react('✊'), msg.react('✌️'), msg.react('✋')]).catch(() => msg.reply('다음에 할래요.'));
            ...
            if (size < 2) {
                ...
                const random = [...users][Math.round(Math.random() * (size - 1))];
        ...
        // Moderation
            ...
            if (member) {
                ...
                if (!action || !split[2] || !split[3])
                    return msg.reply('역할 [행동(추가 / 삭제)] [@유저] [역할 이름]으로 사용하실 수 있어요.');
